...
public class SyncAnswerMode {
    private ApiUrlGenerator apiUrlGenerator = new ApiUrlGenerator();
    private String apiLink;
    private DbHelper dbHelper;
    private Context context;

    public SyncAnswerMode(Context context) {
        this.context = context;
        apiLink = apiUrlGenerator.getAnswerModeApiLink();
        dbHelper = new DbHelper(context);
    }

    public void startDownLoad() {
        if(Build.VERSION.SDK_INT >= 11)
            new SyncAnswerMode.GetJson().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
        else
            new SyncAnswerMode.GetJson().execute();
    }
